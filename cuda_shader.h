...
inline __device__ float3 d_get_normal(const float3 &pos)
{
    ...
    sample1.x = tex3D(texBrickPool, pos.x-.5f, pos.y, pos.z);  //y: alpha value
    sample2.x = tex3D(texBrickPool, pos.x+.5f, pos.y, pos.z);	// Cannot use offset 1.0 to previent referencing out of block blound
    sample1.y = tex3D(texBrickPool, pos.x, pos.y-.5f, pos.z);
    sample2.y = tex3D(texBrickPool, pos.x, pos.y+.5f, pos.z);
    sample1.z = tex3D(texBrickPool, pos.x, pos.y, pos.z-.5f);
    sample2.z = tex3D(texBrickPool, pos.x, pos.y, pos.z+.5f);
